...
public class MetaTileEntityMiner extends WorkableTieredMetaTileEntity {
     ...
     * base work duration
     */
    private static final int BASE_DURATION = 1;
    ...
    class MinerLogic extends RecipeLogicEnergy {
        ...
        protected void trySearchNewRecipe() {
            ...
            if(oreStack.getItem() == DUMMY.getItem()) {
        ...
        protected boolean setupAndConsumeRecipeInputs(Recipe recipe) {
            if(!oreHandler.getStackInSlot(0).isItemEqual(DUMMY) && !oreHandler.getStackInSlot(0).isEmpty()) {
        ...
        protected boolean digDeeper() {
           ...
           if(world.getBlockState(new BlockPos(current.getX(),workingLayer - 1,current.getZ())).getBlock() != NonMetaBlocks.MINING_TUBE) {
               ...
               if (!tubes.isEmpty() && tubes.getItem() == NonMetaBlocks.ITEM_MINING_TUBE) {
                   tubes.shrink(1);
                   world.setBlockState(new BlockPos(current.getX(), workingLayer - 1, current.getZ()), NonMetaBlocks.MINING_TUBE.getDefaultState());
                   workingLayer--;
                   success = true;
               }
           }
        ...
        protected void goNextPosition() {
            if(digZ >= 16) {
                if(digX < 16) {
                    ...
                    if(!notEnoughTubes && !isRetrievingTubes) {
                        forceLoadWorkingChunk();
                    }
                    digX = 0;
                }
    ...
    protected ModularUI createUI(EntityPlayer entityPlayer) {
        ModularUI.Builder builder = ModularUI.defaultBuilder().
                label(0, 0, "采矿机", 0xFFFFFF).
                widget(new SlotWidget(importItems, 0, 79, 5, true, true)
                        .setBackgroundTexture(GuiTextures.SLOT, GuiTextures.IN_SLOT_OVERLAY));
        int outSlotStartX = 34;
        int outSlotStartY = 26;
        for(int row = 0; row < 3; row++) {
            for(int column = 0; column < 6; column++) {
                builder.widget(new SlotWidget(exportItems, row * 6 + column, outSlotStartX + column * 18, outSlotStartY + row * 18, true, false)
                        .setBackgroundTexture(GuiTextures.SLOT, GuiTextures.OUT_SLOT_OVERLAY));
            }
        }
        InvWrapper invWrapper = new InvWrapper(entityPlayer.inventory);
        for(int row = 1; row < 4; row++) {
            for(int column = 0; column < 9; column++) {
                builder.widget(new SlotWidget(invWrapper, row * 9 + column, 7 + column * 18, 84 + (row - 1) * 18, true, true)
                        .setBackgroundTexture(GuiTextures.SLOT, GuiTextures.SLOT));
            }
        }

        for(int column = 0; column < 9; column++) {
            builder.widget(new SlotWidget(invWrapper,  column, 7 + column * 18, 144, true, true)
                    .setBackgroundTexture(GuiTextures.SLOT, GuiTextures.SLOT));
        }
